...
if(isset($_SESSION['cart']) && isset($_SESSION['nameUser'])) {
    ...
    if(mysqli_num_rows($result) > 0) {
       ...
       // $cart_payment=$cart_data['payment'];
        $update_cart_date = "UPDATE tbl_cart SET cart_date = '".$now."'WHERE id_khachhang = '".$id_KH."'";
        $cart_date_query = mysqli_query($mysqli, $update_cart_date);
        if(!$cart_date_query) {
            // Xử lý lỗi khi cập nhật ngày giờ không thành công
            exit("Lỗi khi cập nhật ngày giờ.");
        }
    } else {
